      ...
      <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 shadow-2xl overflow-hidden">
        ...
        <div className="divide-y divide-zinc-100 dark:divide-white/5 max-h-[65vh] overflow-y-auto">
          ...
          <div className="px-5 py-4 space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {booking.clientPhone && (
                  <p className="text-xs text-zinc-400 dark:text-zinc-500">{booking.clientPhone}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-2">
              {booking.clientPhone ? (
                <a
                  ...
                  className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-sm font-medium text-emerald-700 dark:text-emerald-400 transition-colors hover:bg-emerald-500/10"
                >
                  <WhatsAppIcon />
                  WhatsApp
                </a>
                ...
                <span
                  ...
                  className="flex flex-1 cursor-not-allowed items-center justify-center gap-2 rounded-lg border border-zinc-100 dark:border-white/5 bg-zinc-50 dark:bg-white/[0.02] px-3 py-2 text-sm font-medium text-zinc-300 dark:text-zinc-600"
                >
                  <WhatsAppIcon />
                  WhatsApp
                </span>
              ...
              <a
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-lg border border-zinc-200 dark:border-white/10 px-3 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-300 transition-colors hover:bg-zinc-50 dark:hover:bg-white/5"
              >
          ...
          {(booking.eventType ||
            ...
            booking.djPreference) && (
            <div className="px-5 py-4">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                Detalles del evento
              </p>
              <div className="rounded-lg border border-zinc-100 dark:border-white/5 bg-zinc-50 dark:bg-white/[0.02] divide-y divide-zinc-100 dark:divide-white/5 overflow-hidden">
                {booking.eventType && (
                  <div className="flex items-center justify-between px-3 py-2.5">
                    <span className="text-xs text-zinc-500 dark:text-zinc-400">Tipo de evento</span>
                    <span className="text-xs font-medium text-zinc-900 dark:text-white">
                      {booking.eventType === 'otro'
                        ? (booking.eventTypeOther ?? 'Otro')
                ...
                {booking.eventDate && (
                  <div className="flex items-center justify-between px-3 py-2.5">
                    ...
                    <span className="text-xs font-medium capitalize text-zinc-900 dark:text-white">
                      {format(parseISO(booking.eventDate), "d 'de' MMMM yyyy", { locale: es })}
                    </span>
                  </div>
                )}
                {booking.eventTime && (
                ...
                {booking.guestCount != null && (
                  <div className="flex items-center justify-between px-3 py-2.5">
                    <span className="text-xs text-zinc-500 dark:text-zinc-400">Invitados</span>
                    <span className="text-xs font-medium text-zinc-900 dark:text-white">
                      {booking.guestCount}
                    </span>
                  </div>
                ...
                {booking.eventLocation && (
                  <div className="flex items-start justify-between gap-4 px-3 py-2.5">
                    <span className="text-xs text-zinc-500 dark:text-zinc-400 shrink-0">Lugar</span>
                    <span className="text-xs font-medium text-right text-zinc-900 dark:text-white">
                      {booking.eventLocation}
                    </span>
                  </div>
                )}
                {booking.djPreference && (
                  <div className="flex items-center justify-between px-3 py-2.5">
                    <span className="text-xs text-zinc-500 dark:text-zinc-400">DJ preferido</span>
                    <span className="text-xs font-medium text-zinc-900 dark:text-white">
                      {booking.djPreference}
                    </span>
                  </div>
