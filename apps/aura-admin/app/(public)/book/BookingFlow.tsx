      ...
      {step === 1 && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <Field label="WhatsApp *" error={errors.clientPhone?.message}>
            ...
            <PhoneInput onChange={(v) => setValue('clientPhone', v)} inputClass={inputClass} />
          </Field>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Fecha aproximada del evento" error={errors.eventDate?.message}>
              ...
              <EventDatePicker
                ...
                onChange={(v) => {
                  setEventDateValue(v)
                  setValue('eventDate', v || undefined)
                }}
                inputClass={inputClass}
                t={t}
              />
            </Field>
            <Field label="Horario aproximado (opcional)" error={errors.eventTime?.message}>
              <input type="hidden" {...register('eventTime')} />
              <TimePicker
                value={eventTimeValue}
                onChange={(v) => {
                  setEventTimeValue(v)
                  setValue('eventTime', v || undefined)
                }}
                inputClass={inputClass}
                t={t}
              />
